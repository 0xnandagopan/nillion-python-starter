...
async def main(args=None):
    ...
    receipt_store = await get_quote_and_pay(
        writer,
        nillion.Operation.store_values(secrets_object, ttl_days=5),
        payments_wallet,
        payments_client,
        cluster_id,
    )
    ...
    print(
        f"\n📋 python3 03_retrieve_secret.py --store_id {store_id} --secret_name {secret_name_acc}"
    )
    return store_id
